...
def generate_simulated_data():
    ...
    base_demand = 50000 + (np.arange(366) * 50) 
    seasonality = np.where(dates.month >= 11, 20000, 0) # Holiday spike
    noise = np.random.normal(0, 5000, 366)
    daily_demand = base_demand + seasonality + noise
    
    # Simulate KPIs
    ad_spend = daily_demand * np.random.uniform(0.08, 0.12, 366) # 8-12% of demand
    tacos = (ad_spend / (daily_demand * 25)) * 100 # Assuming $25 avg order value
    ipi = 600 + (np.random.normal(0, 20, 366)) - (tacos * 2) # IPI drops slightly if ad efficiency drops
...
st.sidebar.download_button(
    ...
    data=csv,
    file_name='amazon_simulated_data_2024.csv',
    mime='text/csv',
)
...
[Image of project implementation Gantt chart]
